 ...
 * 
 * 1.负责SpyMemcached Client的启动与关闭. 2.提供常用的Get/GetBulk/Set/Delete/Incr/Decr函数的封装.
 * 
 * 未提供封装的函数可直接调用getClient()取出Spy的原版MemcachedClient来使用.
 * 
 * <pre>
 * 	&lt;!-- SpyMemcached客户端 -->
 * 	&lt;bean id="spyClient" class="org.springside.modules.memcached.SpyMemcachedClientWrapper">
 * 		&lt;!-- 设置Memcached集群 -->
...
public class SpyMemcachedClient implements InitializingBean, DisposableBean {
	...
	public void afterPropertiesSet() throws Exception {
		...
		for (int i = 0; i < poolSize; i++) {
			...
			try {
				MemcachedClient spyClient = new MemcachedClient(cfb.build(),
						AddrUtil.getAddresses(memcachedNodes));
