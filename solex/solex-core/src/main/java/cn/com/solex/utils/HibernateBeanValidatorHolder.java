 ...
 * <pre>
 * static 方法调用:
 * HibernateBeanValidatorHolder.validate(object);
 * </pre>
 * 
 * <pre>
 * spring配置:
...
public class HibernateBeanValidatorHolder implements InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
		if (validator == null)
			throw new IllegalStateException(
					"not found JSR303(HibernateValidator) 'validator' for BeanValidatorHolder ");
	}

	public void setValidator(Validator v) {
		if (validator != null) {
			throw new IllegalStateException(
					"BeanValidatorHolder already holded 'validator'");
		}
		validator = v;
	}
	...
	public static Validator getValidator() {
		if (validator == null)
			throw new IllegalStateException(
					"'validator' property is null,BeanValidatorHolder not yet init.");
		return validator;
	}

	public static final <T> Set<ConstraintViolation<T>> validate(T object,
			Class<?>... groups) {
		return getValidator().validate(object, groups);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final <T> void validate(T object)
			throws ConstraintViolationException {
		Set constraintViolations = getValidator().validate(object);
		String msg = "validate failure on object:"
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final <T> void validateProperty(T object, String propertyName)
			throws ConstraintViolationException {
		Set constraintViolations = getValidator().validateProperty(object,
				propertyName);
		String msg = "validate property failure on object:"
				+ object.getClass().getSimpleName() + "." + propertyName + "";
		throw new ConstraintViolationException(msg, constraintViolations);
	}

	public static final <T> Set<ConstraintViolation<T>> validateValue(
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final <T> void validateValue(Class<T> beanType,
			String propertyName, Object value)
			throws ConstraintViolationException {
		Set constraintViolations = getValidator().validateValue(beanType,
				propertyName, value);
		String msg = "validate value failure on object:"
				+ beanType.getSimpleName() + "." + propertyName + " value:"
				+ value;
		throw new ConstraintViolationException(msg, constraintViolations);
	}
