 ...
 * 
 * 在web.xml中配置的参数包括： 1.failureUrl -- 身份验证失败后跳转的URL, 需与SpringSecurity中的配置保持一致,
 * 无默认值必须配置. 2.filterProcessesUrl -- 登录表单处理URL, 需与SpringSecurity中的配置一致,
 * 默认为"/j_spring_security_check". 3.captchaServiceId -- captchaService在Spring
 * ApplicationContext中的bean id,默认为"captchaService". 4.captchaParamter --
 * 登录表单中验证码Input框的名称, 默认为"j_captcha". 5.autoPassValue -- 用于自动化功能测试的自动通过值,
 * 默认该值不存在.
 * 
 * @author xuenong_li
 * 
