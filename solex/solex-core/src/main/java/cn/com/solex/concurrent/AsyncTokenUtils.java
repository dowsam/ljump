...
public class AsyncTokenUtils {
	 ...
	 */
	public static <T> void execute(Executor executor, AsyncToken<T> token,
	 ...
	 * 通过executor执行Callable,将callable的执行结果设置进token
	 */
	public static <T> AsyncToken<T> execute(Executor executor,
			AsyncTokenFactory factory, final Callable<T> task) {
	 ...
	 */
	public static <T> AsyncToken<T> execute(Executor executor,
			AsyncTokenFactory factory, final Runnable task) {
		AsyncToken<T> token = factory.newToken();
		executor.execute(new AsyncTokenRunnable<T>(token, task));
		return token;
	}
